...
def insert_into_transcripts(user_id, file_name, transcription, transcription_summary, sentiment_label):
    with sqlite3.connect('MASTER.db') as conn:
        cursor = conn.cursor()
        query = """
            INSERT INTO Transcripts (user_id, file_name, transcription, transcription_summary, sentiment_label) 
            VALUES (?, ?, ?, ?, ?)
        """
        cursor.execute(query, (user_id, file_name, transcription, transcription_summary, sentiment_label))
        conn.commit()
