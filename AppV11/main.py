...
def main():
    ...
    if st.session_state.user_authenticated:
        ...
        # TODO: Change this to vector DB Approach to allow for more context
        # Insert Prompt initially as memory
        ...
        chat_llm_chain = LLMChain(llm=chat_llm, prompt=chat_template)
            ...
            if uploaded_file is not None:
                if st.button("Generate Transcript and Summary"):
                    ...
                    with st.expander("Previous Related Information (Pinecone Retrieval QA)", expanded=True):
                        st.write(st.session_state.current_ai_research)
        ...
        with prev_tab:
            ...
            if st.session_state.prev_transcript:
                ...
                if st.session_state.prev_transcript_summary:
                    ...
                    with st.expander("Previous Related Information (Pinecone Retrieval QA)", expanded=True):
                        st.write(st.session_state.prev_ai_research)
